...
- **Bias-Variance tradeoff**. This is a fundamental concept in machine learning when we try to balance two competing factors: *bais* and *variance*. 
    * **Low bias, high variance**. The model is complex and captures many details, including noise, leading to overfitting. The prediction can vary depending on the specific training data used, but model can potential performs ver
