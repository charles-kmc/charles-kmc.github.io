...
- **Variance**: Variance referes to how much the model's predictions change when trained on different subsets of data. 
    * **High variance**: High variance often occurs when a the model is too complex relative to the amunt of training data available. A model with high variance will fit well the training data, but when exposed to new, unsee
    * **Low variance**: 
